...
import { ExpenseItem, SimulationData } from '@/lib/ma-simulation'
...
export function ExpenseSection({ data, onChange }: ExpenseSectionProps) {
    const handleDetailChange = (
        ...
        if (type === 'labor') {
            onChange({ ...data, laborDetails: newList })
        } else {
            onChange({ ...data, leaseDetails: newList })
        }
    }
    ...
    const addItem = (type: 'labor' | 'lease') => {
        ...
        if (type === 'labor') {
            onChange({ ...data, laborDetails: [...data.laborDetails, newItem] })
        } else {
            onChange({ ...data, leaseDetails: [...data.leaseDetails, newItem] })
        }
    }

    const removeItem = (type: 'labor' | 'lease', id: string) => {
        if (type === 'labor') {
            onChange({ ...data, laborDetails: data.laborDetails.filter(i => i.id !== id) })
        } else {
            onChange({ ...data, leaseDetails: data.leaseDetails.filter(i => i.id !== id) })
        }
    }
        ...
        <Card>
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
                ...
                <div className="flex items-center space-x-2">
                    <Label htmlFor="detailed-mode" className="text-xs font-normal text-slate-500">詳細入力</Label>
                    <Switch
                        id="detailed-mode"
                        checked={data.useDetailedExpenses}
                        onCheckedChange={(checked) => onChange({ ...data, useDetailedExpenses: checked })}
                    />
                </div>
            </CardHeader>
            <CardContent className="space-y-4">
                ...
                <div>
                    ...
                    <CurrencyInput
                        value={data.rent}
                        onChange={(val) => onChange({ ...data, rent: val })}
                        className="mt-1"
                    />
                </div>
                ...
                <div>
                    ...
                    <CurrencyInput
                        ...
                        onChange={(val) => onChange({ ...data, utilities: val })}
                        className="mt-1"
                    />
                </div>
                ...
                <div className="space-y-2">
                    ...
                    {!data.useDetailedExpenses ? (
                        <CurrencyInput
                            ...
                            onChange={(val) => onChange({ ...data, laborCostTotal: val })}
                        />
                    ) : (
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-center">
                        <Label className="text-sm">その他経費・リース</Label>
                        {data.useDetailedExpenses && (
                    ...
                    {!data.useDetailedExpenses ? (
                        <CurrencyInput
                            ...
                            onChange={(val) => onChange({ ...data, otherExpensesTotal: val })}
                        />
                    ) : (
                        <div className="space-y-2 bg-slate-50 p-3 rounded-md border">
